...
BinaryNode *CopyBinaryTree(BinaryNode *node)
{
    if(node == NULL)
        return NULL;

    // 拷贝左子树
    BinaryNode *lchild = CopyBinaryTree(node->lchild);

    // 拷贝右子树
    BinaryNode *rchild = CopyBinaryTree(node->rchild);
...
void CreateBinaryTree()
{
    ...
    // 拷贝二叉树
    BinaryNode *newTree = CopyBinaryTree(&node1);
    Recursion(newTree);
    FreeSpaceBinaryTree(newTree);
    printf("\n");
}
